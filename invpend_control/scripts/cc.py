...
def exceedRange(pos_cart, pos_pole):
    return math.fabs(pos_cart) > 2.4 or math.fabs(pos_pole) > 2*math.pi # cart: +-2.4; pole: +-45degrees
...
class CartPole(object):
    ...
    def __init__(self):
        ...
        self.setpoint = 0  # Set the target velocity
        self.prev_error = 0  # Previous error for derivative term
        self.integral = 0  # Integral term for integral control

    def pid_control(self):
        """ Control cart with PID velocity command """
        ...
        while not rospy.is_shutdown():
            ...
            if self.out_range:
                ...
                print("=== Within range, exert PID vel command ===")
                current_velocity = ob[1]  # Get the current velocity from the observation

                # Compute PID control output
                error = self.setpoint - current_velocity
                self.integral += error * (1.0 / self.freq)
                derivative = (error - self.prev_error) * self.freq

                vel_cmd = self.Kp * error + self.Ki * self.integral + self.Kd * derivative

                # Take action with the computed velocity command
